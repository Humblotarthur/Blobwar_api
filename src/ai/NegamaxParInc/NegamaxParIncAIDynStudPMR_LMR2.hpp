#pragma once
// NegamaxParIncAIDynStud_PMR_LMR2 — PMR + LMR à 2 classes fixes.
// Classe 0 : coup 0 (séquentiel, pleine profondeur)
// Classe 1 : coups 1..nExplored-1 (parallèle, depth-2)
// Re-search si score réduit > alpha.
...
class NegamaxParIncAIDynStud_PMR_LMR2 : public AIBase {
    using Clock     = std::chrono::steady_clock;
    using TimePoint = Clock::time_point;

public:
    explicit NegamaxParIncAIDynStud_PMR_LMR2(NegamaxConfig cfg = {})
        : cfg_(cfg), ztt_(nullptr), hash_(0), ownsZtt_(false),
          timeUp_(nullptr), ownsTimeUp_(false) {}

    static NegamaxParIncAIDynStud_PMR_LMR2 fromConfigFile(const std::string& path) {
        return NegamaxParIncAIDynStud_PMR_LMR2(NegamaxConfig::loadFromFile(path));
    }

    NegamaxParIncAIDynStud_PMR_LMR2(const NegamaxParIncAIDynStud_PMR_LMR2& o)
    ...
    NegamaxParIncAIDynStud_PMR_LMR2& operator=(const NegamaxParIncAIDynStud_PMR_LMR2& o) {
        cfg_        = o.cfg_;
        state_      = o.state_;
    ...
    ~NegamaxParIncAIDynStud_PMR_LMR2() {
        if (ownsZtt_)    delete ztt_;
        if (ownsTimeUp_) delete timeUp_;
    }

    std::string name() const override {
        return "NegamaxParIncAIDynStud_PMR_LMR2(t=" + std::to_string(cfg_.timeLimitMs) + "ms)";
    }
...
private:
    ...
    // ─── YBW root-parallel + Zobrist TT + PMR (null-window) + LMR 2 classes ───
                              ...
                              int16_t alpha, int16_t beta)
    {
        ...
        // LMR : actif pour tous les coups 1..nExplored-1 (classe 1, réduction = 1)
        const bool lmrActive    = cfg_.lmrEnabled && depth >= cfg_.lmrMinDepth;
        const int  lmrReduction = lmrActive ? 1 : 0;
        ...
        tbb::parallel_for(tbb::blocked_range<int>(1, nExplored),
            [&](const tbb::blocked_range<int>& r) {
                for (int i = r.begin(); i != r.end(); ++i) {
                    ...
                    int8_t blobMask = 0;
                    NegamaxParIncAIDynStud_PMR_LMR2 worker = *this;
                    ...
                    // Recherche réduite (classe 1 : reduction = 1)
                    const int childDepth = std::max(1, depth - 1 - lmrReduction);
                    ...
                    // Re-search à pleine profondeur si le coup semble bon
                    if (lmrReduction > 0 && score > curAlpha && cfg_.lmrResearch) {

#pragma once
// NegamaxParIncAIDynStud — YBWsearchZobristDYN + PMR + LMR K-classes dynamique.
//
// Reconstruction propre depuis YBWsearchZobristDYN.
// K = max(1, floor(log2(nExplored)))
// Partition logarithmique :
//   classe 0  : coup 0 (séquentiel, pleine profondeur — PV node)
//   classe 1  : coups 1..n/2          (reduction = 1)
//   classe 2  : coups n/2+1..3n/4     (reduction = 2)
//   classe k  : ...                   (reduction = k)
// Re-search à pleine profondeur si score réduit > alpha.
//
// Paramètres NegamaxConfig utilisés : timeLimitMs, maxDepth, parallelThreshold,
//   pmrRatio, pmrMinDepth, lmrEnabled, lmrMinDepth, lmrResearch.
...
class NegamaxParIncAIDynStud : public AIBase {
    using Clock     = std::chrono::steady_clock;
    using TimePoint = Clock::time_point;

public:
    explicit NegamaxParIncAIDynStud(NegamaxConfig cfg = {})
        : cfg_(cfg), ztt_(nullptr), hash_(0), ownsZtt_(false),
          timeUp_(nullptr), ownsTimeUp_(false) {}

    static NegamaxParIncAIDynStud fromConfigFile(const std::string& path) {
        return NegamaxParIncAIDynStud(NegamaxConfig::loadFromFile(path));
    }

    NegamaxParIncAIDynStud(const NegamaxParIncAIDynStud& o)
    ...
    NegamaxParIncAIDynStud& operator=(const NegamaxParIncAIDynStud& o) {
        cfg_        = o.cfg_;
        state_      = o.state_;
    ...
    ~NegamaxParIncAIDynStud() {
        if (ownsZtt_)    delete ztt_;
        if (ownsTimeUp_) delete timeUp_;
    }

    std::string name() const override {
        return "NegamaxParIncAIDynStud(t=" + std::to_string(cfg_.timeLimitMs) + "ms)";
    }
...
private:
    ...
    // floor(log2(x)) pour x > 0
    static int ilog2(int x) { return 31 - __builtin_clz(x); }

    // Réduction pour la classe k — customisable en une ligne
    static int reduction(int cls) { return cls; }

    // ─── YBW root-parallel + Zobrist TT + PMR (null-window) + LMR K-classes ───
                              ...
                              int16_t alpha, int16_t beta)
    {
        ...
        // Tri par eval statique (meilleur coup en premier)
        ...
        // Coup 0 : séquentiel, pleine profondeur (classe 0)
        ...
        // LMR K-classes :
        //   K = max(1, floor(log2(nExplored)))
        //   getClass(rank) = K - floor(log2(nExplored - rank))
        const bool lmrActive = cfg_.lmrEnabled && depth >= cfg_.lmrMinDepth && nExplored > 1;
        const int  K         = lmrActive ? std::max(1, ilog2(nExplored)) : 0;

        auto getClass = [&](int rank) -> int {
            // rank dans 1..nExplored-1
            const int remain = nExplored - rank;
            const int cls    = (remain > 0) ? K - ilog2(remain) : K + 1;
            return std::min(K, std::max(1, cls));
        };
        ...
        tbb::parallel_for(tbb::blocked_range<int>(1, nExplored),
            [&](const tbb::blocked_range<int>& r) {
                for (int i = r.begin(); i != r.end(); ++i) {
                    ...
                    int8_t blobMask = 0;
                    NegamaxParIncAIDynStud worker = *this;
                    ...
                    // Réduction selon la classe logarithmique
                    const int cls        = lmrActive ? getClass(i) : 0;
                    const int red        = reduction(cls);
                    const int childDepth = std::max(1, depth - 1 - red);

                    int16_t score = (childDepth <= cfg_.parallelThreshold)
                        ? -worker.searchZobrist(childDepth, opponent(current), -beta, -curAlpha, worker.hash_)
                        : -worker.YBWsearch    (childDepth, opponent(current), -beta, -curAlpha);

                    // Re-search à pleine profondeur si le coup réduit semble bon
                    if (red > 0 && score > curAlpha && cfg_.lmrResearch) {

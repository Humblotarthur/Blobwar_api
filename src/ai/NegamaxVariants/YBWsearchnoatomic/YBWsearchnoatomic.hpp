#pragma once
// Variante archivée : YBWsearchnoatomic — parallèle YBW avec tri, sans atomics
//                     (utilise tbb::parallel_reduce).
...
class NegamaxParIncAI_YBWsearchnoatomic : public AIBase {
public:
    explicit NegamaxParIncAI_YBWsearchnoatomic(int depth = 4)
        : depth_(depth) {}

    std::string name() const override {
        return "NegamaxParInc_YBWsearchnoatomic(d=" + std::to_string(depth_) + ")";
    }
    ...
    Move chooseMove(const Board& b, Player p) override {
        ...
        YBWsearchnoatomic(depth_, p, -10000, 10000);
        return bestMove_;
    }
                              ...
                              int16_t alpha, int16_t beta)
    {
        ...
        int16_t best;
        {
            ...
            best = (depth-1 <= PARALLEL_THRESHOLD)
                 ...
                 : -YBWsearchnoatomic   (depth-1, opponent(current), -beta, -alpha);
        ...
        Result result = tbb::parallel_reduce(
            ...
            [&](const tbb::blocked_range<int>& r, Result local) -> Result {
                for (int i = r.begin(); i != r.end(); ++i) {
                    int8_t blobMask = 0;
                    NegamaxParIncAI_YBWsearchnoatomic worker = *this;
                    ...
                    const int16_t score = (depth-1 <= PARALLEL_THRESHOLD)
                        ...
                        : -worker.YBWsearchnoatomic  (depth-1, opponent(current), -beta, -localAlpha);

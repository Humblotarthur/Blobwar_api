#pragma once
// Variante archivée : YBWsearchZobrist — parallèle YBW + table de transposition Zobrist.
...
class NegamaxParIncAI_YBWsearchZobrist : public AIBase {
public:
    explicit NegamaxParIncAI_YBWsearchZobrist(int depth = 4)
        : depth_(depth), ztt_(nullptr), hash_(0), ownsZtt_(false) {}

    NegamaxParIncAI_YBWsearchZobrist(const NegamaxParIncAI_YBWsearchZobrist& o)
        : depth_(o.depth_), state_(o.state_), bestMove_(o.bestMove_),
          ztt_(o.ztt_), hash_(o.hash_), ownsZtt_(false),
          cfg_(o.cfg_), cnnEval_(o.cnnEval_) {}

    NegamaxParIncAI_YBWsearchZobrist& operator=(const NegamaxParIncAI_YBWsearchZobrist& o) {
        depth_    = o.depth_;
        state_    = o.state_;
    ...
    ~NegamaxParIncAI_YBWsearchZobrist() { if (ownsZtt_) delete ztt_; }

    std::string name() const override {
        return "NegamaxParInc_YBWsearchZobrist(d=" + std::to_string(depth_) + ")";
    }
    ...
    Move chooseMove(const Board& b, Player p) override {
        ...
        YBWsearchZobrist(depth_, p, -10000, 10000);
        return bestMove_;
    }
...
private:
    ...
    inline int16_t leafEval(const BBState& s, Player p) const {
        if (cfg_.useCnnEval && cnnEval_) {
            float raw   = cnnEval_->evaluate(s.bb[0], s.bb[1], s.holes);
            float score = (p == Player::P1) ? raw : -raw;
            return (int16_t)std::clamp(score, -9999.0f, 9999.0f);
        }
        return s.evalFor(p);
    }
        ...
        tbb::parallel_for(tbb::blocked_range<int>(1, n),
            [&](const tbb::blocked_range<int>& r) {
                for (int i = r.begin(); i != r.end(); ++i) {
                    ...
                    int8_t blobMask = 0;
                    NegamaxParIncAI_YBWsearchZobrist worker = *this;

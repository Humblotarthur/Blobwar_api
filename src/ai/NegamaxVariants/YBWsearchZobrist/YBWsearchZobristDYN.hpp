#pragma once
// YBWsearchZobristDYN — YBWsearchZobrist + iterative deepening + time limit.
// Même logique que YBWsearchZobrist (eval sort, Zobrist TT, YBW parallel),
// sans LMR/LMP/sort profond. Sert de référence "propre" pour valider les perf.
...
class NegamaxParIncAI_YBWsearchZobristDYN : public AIBase {
    using Clock     = std::chrono::steady_clock;
    using TimePoint = Clock::time_point;

public:
    explicit NegamaxParIncAI_YBWsearchZobristDYN(int timeLimitMs = 1000, int maxDepth = 20)
        : timeLimitMs_(timeLimitMs), maxDepth_(maxDepth),
          ztt_(nullptr), hash_(0), ownsZtt_(false),
          timeUp_(nullptr), ownsTimeUp_(false) {}

    NegamaxParIncAI_YBWsearchZobristDYN(const NegamaxParIncAI_YBWsearchZobristDYN& o)
    ...
    NegamaxParIncAI_YBWsearchZobristDYN& operator=(const NegamaxParIncAI_YBWsearchZobristDYN& o) {
    ...
    ~NegamaxParIncAI_YBWsearchZobristDYN() {
        if (ownsZtt_)    delete ztt_;
        if (ownsTimeUp_) delete timeUp_;
    }

    std::string name() const override {
        return "YBWsearchZobristDYN(t=" + std::to_string(timeLimitMs_) + "ms)";
    }
...
private:
                                    ...
                                    int16_t alpha, int16_t beta)
    {
        ...
        if (n == 1 || best >= beta) {
            ztt_->store(hash_, best, static_cast<int8_t>(depth), (best >= beta) ? TT_LOWER : TT_EXACT);
            return best;
        }
        ...
        tbb::parallel_for(tbb::blocked_range<int>(1, n),
            [&](const tbb::blocked_range<int>& r) {
                for (int i = r.begin(); i != r.end(); ++i) {
                    ...
                    int8_t blobMask = 0;
                    NegamaxParIncAI_YBWsearchZobristDYN worker = *this;
                    ...
                    const int16_t score = (depth-1 <= PARALLEL_THRESHOLD)
                        ...
                        : -worker.YBWsearchZobrist (depth-1, opponent(current), -beta, -curAlpha);
                                 ...
                                 int16_t alpha, int16_t beta, uint64_t hash)
    {
        ...
        if (!state_.hasAnyMove(pi))
            return -searchZobrist(depth-1, opponent(current), -beta, -alpha, hash ^ ztt_->sideToMove);

#pragma once
// Variante archivée : YBWsearchtrinoYBW — avec tri, sans parallélisme YBW séquentiel.
...
class NegamaxParIncAI_YBWsearchtrinoYBW : public AIBase {
public:
    explicit NegamaxParIncAI_YBWsearchtrinoYBW(int depth = 4)
        : depth_(depth) {}

    std::string name() const override {
        return "NegamaxParInc_YBWsearchtrinoYBW(d=" + std::to_string(depth_) + ")";
    }
    ...
    Move chooseMove(const Board& b, Player p) override {
        ...
        YBWsearchtrinoYBW(depth_, p, -10000, 10000);
        return bestMove_;
    }
        ...
        tbb::parallel_for(tbb::blocked_range<int>(1, n),
            [&](const tbb::blocked_range<int>& r) {
                for (int i = r.begin(); i != r.end(); ++i) {
                    ...
                    int8_t blobMask = 0;
                    NegamaxParIncAI_YBWsearchtrinoYBW worker = *this;
                    ...
                    const int16_t score = (depth-1 <= PARALLEL_THRESHOLD)
                        ...
                        : -worker.YBWsearchtrinoYBW  (depth-1, opponent(current), -beta, -curAlpha);

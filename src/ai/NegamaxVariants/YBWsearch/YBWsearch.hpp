#pragma once
// Variante archivée : YBWsearch — parallèle YBW avec tri heuristique statique.
...
class NegamaxParIncAI_YBWsearch : public AIBase {
public:
    explicit NegamaxParIncAI_YBWsearch(int depth = 4)
        : depth_(depth) {}

    std::string name() const override {
        return "NegamaxParInc_YBWsearch(d=" + std::to_string(depth_) + ")";
    }
    ...
    Move chooseMove(const Board& b, Player p) override {
        ...
        YBWsearch(depth_, p, -10000, 10000);
        return bestMove_;
    }
        ...
        tbb::parallel_for(tbb::blocked_range<int>(1, n),
            [&](const tbb::blocked_range<int>& r) {
                for (int i = r.begin(); i != r.end(); ++i) {
                    ...
                    int8_t blobMask = 0;
                    NegamaxParIncAI_YBWsearch worker = *this;
